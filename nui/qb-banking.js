...
window.addEventListener("message", function (event) {
    if(event.data.status == "openbank") {
        ...
        $("#bankingUserInfo-tab").removeClass('active');
        ...
        $("#bankingUserInfo").removeClass('active').removeClass('show');
...
function populateBanking(data)
{
    ...
    $("#currentBalance3").html(data.bankbalance);
    $("#currentCashBalance3").html(data.cash);
